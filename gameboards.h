...
const KlotskiBoard HrdBoard = KlotskiBoard(HrdBlocks, HrdBlanks, QString::fromLocal8Bit("华容道"), QString::fromLocal8Bit("最早记载于西北工业大学教授姜长英在1949年出版的《科学消遣》，大致推测出该版本的样式最早始于民国。创意来源于《三国演义》中“曹操败走华容道”的情景，所以游戏的目标就是让曹操从华容道逃走，体现在棋盘上的就是把最大的一块移到底部中间。而在此要把“曹操”这个棋子移动到出口处，真得靠“关羽”这个棋子让路，和《三国演义》中的故事很贴合。"), "HRD", 4, 5);
...
const KlotskiBoard ShogiBoard = KlotskiBoard(ShogiBlocks, ShogiBlanks, QString::fromLocal8Bit("将棋"), QString::fromLocal8Bit("在日本主要有两种外观的版本：箱种女孩和将棋，因为前者的模式跟“华容道”一样，我在此选的是后者。将棋棋子样式的外观。目标是把“王将”也就是最大的一块移到底部中间。"), "Shogi", 4, 5);
...
const KlotskiBoard DonaldBoard = KlotskiBoard(DonaldBlocks, DonaldBlanks, QString::fromLocal8Bit("唐老鸭"), QString::fromLocal8Bit("这是我从一本书上看到的外观是迪士尼人物的版本。目标是让唐老鸭踢进球门，所以也是把最大的一块移到底部中间。从画风来看，该版本并没有得到迪士尼的授权，我在此用的是搜索到的可对应上的迪士尼人物图片，不过为了方便米奇和高飞都只有两种动作。"), "Donald", 4, 5);
...
const KlotskiBoard TenBoard = KlotskiBoard(TenBlocks, TenBlanks, QString::fromLocal8Bit("10 Block"), QString::fromLocal8Bit("完整的描述是：10 Block Puzzle by A. Filipiak。目标是把最大的两块也就是A块和B块的位置交换。"), "10Block", 4, 5);
...
const KlotskiBoard BlockadoBoard = KlotskiBoard(BlockadoBlocks, BlockadoBlanks, QString::fromLocal8Bit("Blockado"), QString::fromLocal8Bit("完整的描述是：Blockado by Acme Novelty Sales。目标是把最大的一块移到左下角。比较少有的尺寸是4×6的棋盘。"), "Blockado", 4, 6);
...
const KlotskiBoard PennantBoard = KlotskiBoard(PennantBlocks, PennantBlanks, QString::fromLocal8Bit("Pennant"), QString::fromLocal8Bit("完整的描述是：Pennant Puzzle by L. W. Hardy，又名：Dad's Puzzler。目标是把最大的一块移到左下角。该谜题始于1909年，后来出过多种外观样式的变种，在此用的是最早的一种印有美国地名的。"), "Pennant", 4, 5);
